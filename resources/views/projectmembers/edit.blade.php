@extends('layouts.app')
@section('title', 'プロジェクトメンバー情報を更新する')

@section('content')
<div class="container">

	<form action="{{ url('settings/projects/'.urlencode($project->id).'/members/'.urlencode($user->id)) }}" method="post">
		@csrf
		@method('PUT')
				...
				<tr>
					...
					<td>
						<select id="role" class="form-control{{ $errors->has('role') ? ' is-invalid' : '' }}" name="role" value="{{ old('role', $relation->role) }}" required>
							<?php $vals = array( 'observer', 'partner', 'member', 'manager', 'owner' ); ?>
							@foreach ($vals as $val)
	...
	<div class="text-right">
		<a href="{{ url('settings/projects/'.urlencode($project->id).'/members/'.urlencode($user->id)) }}" class="btn btn-default">キャンセル</a>
	</div>
